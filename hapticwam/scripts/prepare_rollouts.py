...
def build(task, model, episode, verdicts, maximum):
    ...
    dest = OUT / f'rollout-heatmap-{task}-{model}.mp4'
    ...
    for i in range(len(times)):
        ...
        if i == 0:
            canvas.save(OUT / f'rollout-heatmap-{task}-{model}.webp', quality=90)
    ...
    entry = {'task': task, 'model': model, 'episode': episode, 'seed': 101,
             'video': 'assets/' + dest.name, 'poster': f'assets/rollout-heatmap-{task}-{model}.webp',
...
def main():
    ...
    maxima = {task: float(np.ceil(max(ranges[e] for e in pair) * 10) / 10) for task, pair in PAIRS.items()}
    with ThreadPoolExecutor(max_workers=2) as pool:
        entries = list(pool.map(lambda args: build(*args, verdicts, maxima[args[0]]), jobs))
